...
\indent{Each data file should also have an associated help page. The easiest way to generate a help page is to use \texttt{roxygen2} and a dummy R function. Typically, I have a file called \texttt{data\_help\_files.R}, which has entries for the each data set. For example,}
...
the demos\sidenote{There is a planned \texttt{demoTitle} tag for \texttt{roxygen2}, but currently this hasn’t been implemented.}. For example,\sidenote{Note the white space separation in
